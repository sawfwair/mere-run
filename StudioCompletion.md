...
| 4.7 | ✅ **Window/menu/restore (done)** — selected mode + panel layout persist per scene via `@SceneStorage` (relaunch reopens the last mode; conversational modes re-resume the latest thread on launch); a View menu drives Library/Advanced toggles + Browse Models on the key Studio window via `.focusedSceneValue`/`@FocusedValue` (⌃⌘L / ⌃⌘E / ⇧⌘M); the Run menu already exposes Open/Reveal Last Output. _(Draft serialization across relaunch deliberately deferred — the composer is per-mode and conversational modes reopen their own thread.)_ | M | Standard menus work; relaunch restores the last mode and layout. |
| 4.8 | ✅ **Clipboard paste (done)** — image-accepting modes get an explicit composer Paste button plus ⌘V via `.onPasteCommand` (pasted file used directly, else the bitmap written to a temp PNG); the file drop target shows an animated dashed accent highlight while a drag hovers. | S | Pasting an image populates the attachment. |
