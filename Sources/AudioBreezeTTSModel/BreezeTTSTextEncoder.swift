...
final class BreezeTTSTextAttention: Module {
    ...
    init(config: BreezeTextEncoderConfig, layerIndex: Int) {
        ...
        let scaling = layerType == "full_attention"
            ? BreezeRopeScaling(factor: 8, highFreqFactor: nil, lowFreqFactor: nil,
                                 originalMaxPositionEmbeddings: nil, ropeType: "linear")
            : nil
        self.rope = BreezeRotary(dimensions: config.headDim, theta: ropeBase, scaling: scaling)
