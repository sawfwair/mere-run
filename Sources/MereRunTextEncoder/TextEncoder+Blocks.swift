...
public final class QwenAttention: Module {
  ...
  package func callAsFunction(
    ...
    var projected = output.asType(queries.dtype)
    projected = projected.transposed(0, 2, 1, 3).reshaped(B, L, -1)

    let result = oProj(projected)
    if debug {
        eval(result)
...
public final class QwenEncoder: Module {
  ...
  @ModuleInfo(key: "layers") package var layers: [QwenEncoderLayer]
