...
enum VisionTowerError: LocalizedError {
  case invalidPatchInputShape([Int])
  case gridBatchMismatch(gridCount: Int, batchSize: Int)
  case missingLearnedPositionEmbedding
  case invalidMergedTokenCount(tokens: Int, mergeUnit: Int)
  case unexpectedMergedFeatureDimension(actual: Int, expected: [Int])
  case windowIndexOutOfBounds

  var errorDescription: String? {
    switch self {
    case .invalidPatchInputShape(let shape):
      return "Expected vision patch input shape [batch, tokens, patchVolume]; got \(shape)."
    case .gridBatchMismatch(let gridCount, let batchSize):
      return "Vision grid metadata count (\(gridCount)) does not match batch size (\(batchSize))."
    case .missingLearnedPositionEmbedding:
      return "Qwen3-VL requires learned position embeddings, but pos_embed was not initialized."
    case .invalidMergedTokenCount(let tokens, let mergeUnit):
      return "Vision token count \(tokens) must be divisible by merge unit \(mergeUnit)."
    case .unexpectedMergedFeatureDimension(let actual, let expected):
      return "Unexpected vision feature dimension \(actual); expected one of \(expected)."
    case .windowIndexOutOfBounds:
      return "Vision window index was out of bounds."
    }
  }
}

package struct QwenVisionTowerOutput {
  package let hiddenStates: MLXArray
  package let cumulativeWindowSequenceLengths: MLXArray
  package let cumulativeSequenceLengths: MLXArray
  package let windowIndex: MLXArray
  package let rotaryCos: MLXArray
  package let rotarySin: MLXArray
  package let patchInputs: MLXArray
  /// Qwen3-VL: deepstack visual features from intermediate layers
  package let deepstackFeatures: [MLXArray]
}
...
package final class QwenVisionTower: Module {
  ...
  package init(configuration: QwenVisionConfiguration) {
  ...
  package func callAsFunction(patchInputs: MLXArray, grid: [QwenVisionGrid]) throws -> QwenVisionTowerOutput {
    ...
    guard patchInputs.ndim == 3 else {
      throw VisionTowerError.invalidPatchInputShape(patchInputs.shape)
    }
    guard grid.count == patchInputs.dim(0) else {
      throw VisionTowerError.gridBatchMismatch(gridCount: grid.count, batchSize: patchInputs.dim(0))
    }
    ...
    for entry in grid {
      ...
      let merged = try patchMerger(hidden)
  ...
  private func forwardQwen3(patchInputs: MLXArray, grid: [QwenVisionGrid]) throws -> QwenVisionTowerOutput {
    ...
    guard let posEmbed = posEmbed else {
      throw VisionTowerError.missingLearnedPositionEmbedding
    }
    ...
    for (layerIdx, block) in blocks.enumerated() {
      ...
      if let mergerIdx = deepstackVisualIndexes.firstIndex(of: layerIdx) {
        ...
        let feature = try deepstackMergerList[mergerIdx](h2d)
    ...
    // Merge patches
    let merged = try patchMerger(hiddenStates)
  ...
  package static func prepareQwen3Rotary(rotaryPosEmb: MLXArray) -> (cos: MLXArray, sin: MLXArray) {
  ...
  package func updatePatchEmbed(parameters: ModuleParameters) {
    patchEmbed.update(parameters: parameters)
  }

  package func updatePatchMerger(parameters: ModuleParameters) {
  ...
  package func updateBlock(at index: Int, parameters: ModuleParameters) {
