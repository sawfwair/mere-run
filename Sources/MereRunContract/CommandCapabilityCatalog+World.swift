...
private typealias W = MereRunCapabilityCatalog.WorldServeFamily
    ...
    public static let worldServe = MereRunCommandCapability(
        ...
        options: [
            .init(
                ...
                tier: .expert
            ).scoped(W.only(.dreamX, ignoredBy: [.cosmos3])),
            .init(
            ...
            .init(flag: "--backend", label: "Backend", kind: .choice, choices: ["dreamx", "cosmos3"], defaultValue: "dreamx"),
            .init(flag: "--base-model", label: "Base model", kind: .string).scoped(W.only(.dreamX, ignoredBy: [.cosmos3])),
