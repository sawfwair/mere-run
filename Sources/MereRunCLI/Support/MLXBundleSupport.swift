...
enum MLXBundleSupport {
    ...
    struct MetallibMismatch: Equatable {
      let field: String
      let stamped: String
      let expected: String
    }
    ...
    static let expectedProvenance = MetallibProvenance(
      coreVersion: "0.32.1",
      swiftRevision: "4c4e3d088071050f3c3960bd454831da88ebbcf7",
    ...
    /// and the exact MLX dependency graph this binary was compiled against.
    /// `swift build` never regenerates the metallib, so a leftover library
    /// from an older mlx-swift silently corrupts inference.
    enum MetallibStamp: Equatable {
      case matched
      case unstamped(missingFields: [String])
      case mismatched([MetallibMismatch])
    }
    ...
    static func ensureAvailable(quiet: Bool) throws {
      ...
      if ProcessInfo.processInfo.environment["DYLD_FRAMEWORK_PATH"] != nil {
        if !quiet {
          CLIStderr.write(
            "[mererun] warning: \(bundleName) not found near executable; relying on DYLD_FRAMEWORK_PATH.\n"
          )
        }
        return
      }
    ...
    private static func validateFlatResources(
      ...
      case .unstamped(let missingFields):
        ...
        if let replacement = locateCandidateBundle(executableDir: executableDir, matchedOnly: true)
        {
          ...
          CLIStderr.write(
            "[mererun] Replaced unstamped MLX metallib in \(resourcesURL.path) with stamped copy from \(replacement.path).\n"
          )
          return
        }
        if mismatchOverrideEnabled {
          CLIStderr.write(
            "[mererun] WARNING: running with an incomplete MLX metallib stamp (missing \(missingFields.joined(separator: ", "))) because MERERUN_ALLOW_METALLIB_MISMATCH=1. Expect corrupted output.\n"
          )
          return
        }
        throw unstampedError(location: resourcesURL, missingFields: missingFields)

      case .mismatched(let mismatches):
        if let replacement = locateCandidateBundle(executableDir: executableDir, matchedOnly: true)
        {
          try installCompatibilityMetallibs(bundleURL: replacement, executableDir: executableDir)
          CLIStderr.write(
            "[mererun] Replaced stale MLX metallib (\(mismatchSummary(mismatches))) with matching copy from \(replacement.path).\n"
          )
          return
        }
        ...
        if mismatchOverrideEnabled {
          CLIStderr.write(
            "[mererun] WARNING: running with a mismatched MLX metallib (\(mismatchSummary(mismatches))) because MERERUN_ALLOW_METALLIB_MISMATCH=1. Expect corrupted output.\n"
          )
          return
        }

        throw mismatchError(location: resourcesURL, mismatches: mismatches)
      }
    }
    ...
    private static func validateAndFinish(
      ...
      case .unstamped(let missingFields):
          ...
          let replacement = locateCandidateBundle(executableDir: executableDir, matchedOnly: true)
        {
          ...
          CLIStderr.write(
            "[mererun] Replaced unstamped MLX metallib bundle with stamped copy from \(replacement.path).\n"
          )
          try installCompatibilityMetallibs(bundleURL: bundleURL, executableDir: executableDir)
          return
        }
        guard mismatchOverrideEnabled else {
          throw unstampedError(location: bundleURL, missingFields: missingFields)
        }
        CLIStderr.write(
          "[mererun] WARNING: running with an incomplete MLX metallib stamp (missing \(missingFields.joined(separator: ", "))) because MERERUN_ALLOW_METALLIB_MISMATCH=1. Expect corrupted output.\n"
        )
      ...
      case .mismatched(let mismatches):
        ...
        throw mismatchError(location: bundleURL, mismatches: mismatches)
      }
    }

    private static var mismatchOverrideEnabled: Bool {
      let raw = (ProcessInfo.processInfo.environment["MERERUN_ALLOW_METALLIB_MISMATCH"] ?? "")
        .lowercased()
      return raw == "1" || raw == "true" || raw == "yes"
    }

    private static func validateLinkedRuntime() throws {
      guard let linked = MLXRuntimeVersion.coreVersion.flatMap(normalizedVersion) else {
        throw ValidationError(
          "Unable to read the linked MLX core version; refusing to load an unvalidatable Metal library."
        )
      }
      guard linked == expectedProvenance.coreVersion else {
        throw ValidationError(
          """
          MLX runtime provenance mismatch.

            linked mlx core: \(linked)
            compiled expectation: \(expectedProvenance.coreVersion)

          Resolve the pinned dependencies and rebuild mere.run.
          """
        )
      }
    }
    ...
    private static func stampStatus(resourcesURL: URL) -> MetallibStamp {
      ...
      guard let contents = try? String(contentsOf: stampURL, encoding: .utf8) else {
        return .unstamped(missingFields: [stampName])
      }
      return stampStatus(contents: contents)
    }
    ...
    static func stampStatus(contents: String) -> MetallibStamp {
      ...
      let missingFields = fields.compactMap { field, value in
        value == nil ? field : nil
      }
      ...
      let mismatches = comparisons.compactMap { field, actual, required in
        actual == required
          ? nil : MetallibMismatch(field: field, stamped: actual, expected: required)
      }
    ...
    private static func unstampedError(location: URL, missingFields: [String]) -> ValidationError {
      ValidationError(
        """
        Incomplete MLX Metal shader provenance.

          missing stamp fields: \(missingFields.joined(separator: ", "))
          location: \(location.path)

        An unstamped metallib may silently corrupt inference. Rebuild the
    ...
    private static func mismatchError(
      location: URL,
      mismatches: [MetallibMismatch]
    ) -> ValidationError {
      ...
      return ValidationError(
        ...
        A mismatched metallib may silently corrupt inference. Rebuild it
        from the exact pinned checkout:
    ...
    private static func mismatchSummary(_ mismatches: [MetallibMismatch]) -> String {
      mismatches.map { "\($0.field) \($0.stamped) != \($0.expected)" }.joined(separator: "; ")
    }
    ...
    private static func installCompatibilityMetallibs(bundleURL: URL, executableDir: URL) throws {
      ...
      let bundleResources =
        ...
        .appendingPathComponent("Contents", isDirectory: true)
        .appendingPathComponent("Resources", isDirectory: true)
      let sourceMetallib = bundleResources.appendingPathComponent(
        "default.metallib", isDirectory: false)
      ...
      if fm.fileExists(atPath: sourceStamp.path) {
        try? refreshCopy(
          from: sourceStamp, to: resourcesDir.appendingPathComponent(stampName, isDirectory: false))
      }
    }
    ...
    private static func refreshCopy(from source: URL, to destination: URL) throws {
      ...
      let tmp =
        ...
        .appendingPathComponent(
          ".\(destination.lastPathComponent).tmp-\(getpid())", isDirectory: false)
    ...
    private static func findMlxSwiftBundles(workspaceRoot: URL) -> [URL] {
      ...
      let vendoredBundle = workspaceRoot.appendingPathComponent(
        "vendor/mlx-swift_Cmlx.bundle", isDirectory: true)
      ...
      for dir in derivedDataDirs {
        let candidates: [URL] = [
          dir.appendingPathComponent(
            "Build/Products/Debug/mlx-swift_Cmlx.bundle", isDirectory: true),
          dir.appendingPathComponent(
            "Build/Products/Release/mlx-swift_Cmlx.bundle", isDirectory: true),
        ]
