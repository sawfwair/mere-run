...
The small tier is the tool-capable native `text-agent-ornith-9b`. GGUF models
such as North Mini and Ornith 35B remain available for direct coding
experiments, but the `text-code` API lane rejects tool calls and cannot host
the Pi setup agent.
...
- Use `--agent-model small` for the smallest tool-capable setup model.
