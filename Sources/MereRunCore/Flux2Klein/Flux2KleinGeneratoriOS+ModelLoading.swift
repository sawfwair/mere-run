...
extension Flux2KleinGeneratoriOS {
    ...
    private func loadTransformerFromFile(
        ...
        guard fileData.count >= 8 else {
            throw NSError(domain: "Flux2KleinGeneratoriOS", code: 2, userInfo: [NSLocalizedDescriptionKey: "Invalid safetensors file: too small"])
        }

        let headerSize = fileData.withUnsafeBytes { ptr -> UInt64 in
            ptr.load(as: UInt64.self)
        }

        guard fileData.count >= 8 + Int(headerSize) else {
            throw NSError(domain: "Flux2KleinGeneratoriOS", code: 3, userInfo: [NSLocalizedDescriptionKey: "Invalid safetensors file: header truncated"])
        }

        let headerData = fileData.subdata(in: 8..<(8 + Int(headerSize)))
        guard let headerJSON = try JSONSerialization.jsonObject(with: headerData) as? [String: Any] else {
            throw NSError(domain: "Flux2KleinGeneratoriOS", code: 4, userInfo: [NSLocalizedDescriptionKey: "Invalid safetensors header JSON"])
        }

        let dataOffset = 8 + Int(headerSize)

        // Check if quantized by looking for .scales keys
        let isQuantized = headerJSON.keys.contains { $0.hasSuffix(".scales") }
        ...
        if isQuantized {
            ...
            try applyWeightsStreaming(headerJSON: headerJSON, fileData: fileData, dataOffset: dataOffset, to: transformer)
            return transformer
        }
    }
    ...
    private func applyWeightsStreaming(
        ...
        for (key, metadata) in headerJSON {
            guard key != "__metadata__",
                  let tensorMeta = metadata as? [String: Any],
                  let shape = tensorMeta["shape"] as? [Int],
                  let dtypeStr = tensorMeta["dtype"] as? String,
                  let offsets = tensorMeta["data_offsets"] as? [Int],
                  offsets.count == 2 else {
                continue
            }

            let dtype = safetensorsDTypeToDType(dtypeStr)
            let start = dataOffset + offsets[0]
            let end = dataOffset + offsets[1]
            let tensorData = fileData.subdata(in: start..<end)

            let array = MLXArray(tensorData, shape, dtype: dtype)
    ...
    private func buildQuantizedTransformer(
        ...
        func loadTensor(_ key: String) -> MLXArray? {
            guard let meta = headerJSON[key] as? [String: Any],
            ...
            let end = dataOffset + offsets[1]
            let tensorData = fileData.subdata(in: start..<end)
            return MLXArray(tensorData, shape, dtype: safetensorsDTypeToDType(dtypeStr))
        }
    ...
    /// Convert safetensors dtype string to MLX DType
    private func safetensorsDTypeToDType(_ dtype: String) -> DType {
        switch dtype {
