...
extension ACEStepPipeline {
    func normalizeSourceLatents(_ sourceLatents25Hz: MLXArray?, targetFrames: Int) -> MLXArray {
        let B = 1
        ...
        precondition(
            sourceLatents25Hz.ndim == 3 && sourceLatents25Hz.dim(0) == B && sourceLatents25Hz.dim(2) == decoderConfig.audioAcousticHiddenDim,
            "sourceLatents25Hz must be [1, T, \(decoderConfig.audioAcousticHiddenDim)]"
        )
    ...
    func preparePromptConditionInputs(
        ...
        instruction: String,
        isCover: Bool
    ) -> ACEStepConditionInputs {
        let B = srcLatents.dim(0)
        let T = srcLatents.dim(1)
        precondition(B == 1, "preparePromptConditionInputs currently supports batch size 1.")
        precondition(chunkChannels > 0, "Expected chunk mask channels > 0.")
        ...
        let timbre = packReferenceTimbreLatents(
    ...
    func packReferenceTimbreLatents(
    ...
    ) -> (packed: MLXArray, orderMask: MLXArray) {
        ...
        if let referenceTimbreLatents25Hz, !referenceTimbreLatents25Hz.isEmpty {
            ...
            references = referenceTimbreAudio48kHz.map { waveform in
                let normalizedAudio = normalizeReferenceTimbreAudio(waveform)
        ...
        if references.isEmpty {
            let fallback = normalizeReferenceTimbreLatents(
                fallbackLatents25Hz,
        ...
        let packed = MLX.concatenated(
            references.map {
                normalizeReferenceTimbreLatents(
                    $0,
                    targetFrames: targetFrames,
                    latentDim: latentDim
                )
    ...
    func normalizeReferenceTimbreAudio(_ audio: MLXArray) -> MLXArray {
        ...
        if audio.ndim == 1 {
            ...
            if audio.dim(1) == decoderConfig.audioAcousticHiddenDim {
                preconditionFailure("referenceTimbreAudio48kHz expects waveform tensors, but received latent-shaped [T,\(decoderConfig.audioAcousticHiddenDim)].")
            }
            if audio.dim(1) == vaeConfig.audioChannels {
                ...
                preconditionFailure("referenceTimbreAudio48kHz 2D tensors must be [S,\(vaeConfig.audioChannels)] or [\(vaeConfig.audioChannels),S].")
            }
        } else if audio.ndim == 3 {
            precondition(audio.dim(0) == 1, "referenceTimbreAudio48kHz batch dimension must be 1 when using 3D tensors.")
            ...
            preconditionFailure("referenceTimbreAudio48kHz 3D tensors must be [1,S,\(vaeConfig.audioChannels)] or [1,\(vaeConfig.audioChannels),S].")
        } else {
            preconditionFailure("referenceTimbreAudio48kHz tensors must be 1D, 2D, or 3D.")
        }
    ...
    func normalizeReferenceTimbreLatents(
        ...
        if latents25Hz.ndim == 3 {
            precondition(latents25Hz.dim(0) == 1, "Reference timbre latent batch dimension must be 1.")
            precondition(latents25Hz.dim(2) == latentDim, "Reference timbre latent dim must be \(latentDim).")
            normalized = latents25Hz
        } else if latents25Hz.ndim == 2 {
            precondition(latents25Hz.dim(1) == latentDim, "Reference timbre latent dim must be \(latentDim).")
            normalized = latents25Hz.reshaped(1, latents25Hz.dim(0), latentDim)
        } else {
            preconditionFailure("Reference timbre latents must be [1,T,\(latentDim)] or [T,\(latentDim)].")
        }
