...
public final class SAM31VideoTracker: @unchecked Sendable {
    ...
    public func track(
        ...
        let seedMaskDirectory = maskOutputDirectoryURL?.appendingPathComponent(String(format: "frame_%05d", initFrameIndex), isDirectory: true)
        let seedRun = try segmenter.segment(
            imageURL: asset.frameURLs[initFrameIndex],
            promptSet: promptSet,
            annotatedImageURL: annotatedDir.appendingPathComponent(String(format: "frame_%05d", initFrameIndex)).appendingPathExtension("png"),
            jsonOutputURL: frameJSONDir.appendingPathComponent(String(format: "frame_%05d", initFrameIndex)).appendingPathExtension("json"),
            threshold: threshold,
            resolution: resolution,
            showBoxes: showBoxes,
            showLabels: showLabels,
            multimask: false,
            maskOutputDirectoryURL: seedMaskDirectory
        )
        annotatedFrameURLs[initFrameIndex] = seedRun.annotatedImageURL
        ...
        resultsByFrame[initFrameIndex] = trackingStates.map { $0.seedResult }

        if trackingStates.isEmpty {
            for frameIndex in asset.frameURLs.indices where frameIndex != initFrameIndex {
        ...
        try propagate(
            frameIndices: Array((initFrameIndex + 1)..<asset.frameURLs.count),
        ...
        if initFrameIndex > 0 {
            try propagate(
                frameIndices: Array(stride(from: initFrameIndex - 1, through: 0, by: -1)),
                frameURLs: asset.frameURLs,
                annotatedFrameURLs: &annotatedFrameURLs,
                resultsByFrame: &resultsByFrame,
                trackingStates: trackingStates,
