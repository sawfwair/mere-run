...
public enum ManagedModelAPITask: String, Hashable, Sendable {
    ...
    case textClassifications = "text.classifications"
...
public extension ManagedModelAPIProfile {
    ...
    static func companion(
        ...
        case .textClassify:
            return ManagedModelAPIProfile(
                task: .textClassifications, inputModalities: [.text], outputModalities: [.text],
                contextWindow: 512, maximumOutputTokens: 0, structuredOutput: true
            )
        case .textEmbed:
            return ManagedModelAPIProfile(
                task: .embeddings,
        ...
        case .visionDepth:
            // The depth API currently runs video depth. A still-image depth model
            // declares `apiAvailability: .cliOnly` on its spec instead of being
