...
public enum MereRunModelSourceConfiguration {
    ...
    public static let archiveSHA256sEnvironmentKey = "MERERUN_MODEL_SOURCE_SHA256S"
    ...
    public static func archiveSHA256(
        for key: String,
        environment: [String: String] = ProcessInfo.processInfo.environment
    ) -> String? {
        let normalizedRequestedKey = normalizedArchiveKey(key)
        guard let raw = environment[archiveSHA256sEnvironmentKey] else {
            return nil
        }

        for entry in raw.split(whereSeparator: { ",;\n".contains($0) }) {
            let parts = entry.split(separator: "=", maxSplits: 1)
            guard parts.count == 2 else { continue }
            let configuredKey = normalizedArchiveKey(String(parts[0]))
            guard configuredKey == normalizedRequestedKey else { continue }
            let digest = String(parts[1]).trimmingCharacters(in: .whitespacesAndNewlines)
    ...
    private static func normalizedArchiveKey(_ key: String) -> String {
