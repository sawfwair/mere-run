...
public enum R2DownloadRequestBuilder {
    ...
    /// 1) `MERERUN_R2_SIGNED_URL_ENDPOINT` / `R2_SIGNED_URL_ENDPOINT` (fail-closed control-plane signer)
    ...
    public static func makeGETRequest(
        ...
        if let signedURLEndpoint = firstNonEmpty(environment, keys: ["MERERUN_R2_SIGNED_URL_ENDPOINT", "R2_SIGNED_URL_ENDPOINT"]) {
            ...
            let allowSignedURLFallback = boolFlag(environment, keys: [
                "MERERUN_R2_SIGNED_URL_ALLOW_FALLBACK",
                "R2_SIGNED_URL_ALLOW_FALLBACK"
            ])
            do {
                ...
                if requireSignedURL || !allowSignedURLFallback {
