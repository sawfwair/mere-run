...
enum InklingTextLoRAAdapterError: Error, LocalizedError, Sendable {
    case noMatchingWeights([String])

    var errorDescription: String? {
        switch self {
        case .noMatchingWeights(let keys):
            return "Inkling text LoRA adapter did not match injected LoRA layers. Weight keys: \(keys.prefix(8).joined(separator: ", "))"
        }
    }
}

struct InklingTextLoRAApplyReport: Sendable, Equatable {
    let matchedLayerCount: Int
    let injectedLayerCount: Int
}

enum InklingTextLoRAAdapter {
    static func apply(
        _ lora: LoRA,
        to model: InklingLanguageModel,
        targetSuffixes: [String]? = nil
    ) async throws -> InklingTextLoRAApplyReport {
        ...
        guard matched > 0 else {
            throw InklingTextLoRAAdapterError.noMatchingWeights(weights.weights.keys.sorted())
        }
        eval(layers.values.flatMap { [$0.loraDown, $0.loraUp] })
        return InklingTextLoRAApplyReport(
