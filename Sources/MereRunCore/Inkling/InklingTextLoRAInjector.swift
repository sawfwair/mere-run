...
public enum InklingTextLoRAInjectionError: Error, LocalizedError, Sendable {
    ...
    public var errorDescription: String? {
        ...
        case .noMatchingLayers(let suffixes):
            return "No matching Inkling text Linear layers found for LoRA injection. Target suffixes: \(suffixes.joined(separator: ","))"
        }
    }
}

public enum InklingTextLoRAInjector {
    ...
    public static func inject(
        ...
        guard rank >= 1 else {
            throw InklingTextLoRAInjectionError.invalidRank(rank)
        }
        ...
        for (path, module) in model.leafModules().flattened() {
            ...
            if let existing = module as? InklingSharedOuterLoRASwitchLinear {
                loraLayers[path] = existing
                continue
            }

            if let switchLinear = module as? Q35SwitchLinear {
                let wrapped = InklingSharedOuterLoRASwitchLinear(
                    base: switchLinear,
                    rank: rank,
                    alpha: alphaValue,
                    sharedFactor: path.hasSuffix(".down_proj") ? .output : .input,
        ...
        guard !loraLayers.isEmpty else {
            throw InklingTextLoRAInjectionError.noMatchingLayers(suffixes)
        }
