...
public enum SafetensorsStreamingLoader {
    ...
    package static func metadata(fileData data: Data, fileURL url: URL) throws -> [String: TensorMetadata] {
    ...
    private struct SafetensorsHeader: Decodable {
        ...
        init(from decoder: Decoder) throws {
            let container = try decoder.container(keyedBy: HeaderCodingKey.self)
            var tensors: [String: TensorHeader] = [:]
            tensors.reserveCapacity(container.allKeys.count)

            let metadataKey = HeaderCodingKey("__metadata__")
...
private struct HeaderCodingKey: CodingKey, Sendable {
