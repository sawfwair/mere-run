...
package final class Q38QSACache: KVCache {
    package let attention: KVCache
    ...
    package init(attention: KVCache = KVCacheSimple(), indexer: KVCache = KVCacheSimple()) {
        self.attention = attention
        self.indexer = indexer
    }

    package var offset: Int { attention.offset }
    package var rowOffsets: [Int]? { attention.rowOffsets }
    package var supportsVariablePositionBatching: Bool { attention.supportsVariablePositionBatching }
    ...
    package func makeMask(n: Int) -> MLXFast.ScaledDotProductAttentionMaskMode {
        attention.makeMask(n: n)
    }

    package func fork() -> KVCache {
    ...
    package func batched(with caches: [KVCache]) -> KVCache? {
    ...
    package func unbatchedRows(count: Int) -> [KVCache]? {
