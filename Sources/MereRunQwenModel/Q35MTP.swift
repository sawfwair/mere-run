...
package struct Q35MTPDraftOutput {
    package let logitsHidden: MLXArray
    package let recurrentHidden: MLXArray
}

package protocol Q35MTPDraftModel: AnyObject {
...
package final class Q35MTPModel: Module, Q35MTPDraftModel {
    ...
    package var expertCount: Int { layers.first?.mlp.experts?.expertCount ?? 0 }

    package let diagnosticsID = "qwen-mtp"

    package init(config: Q35Config) {
        let text = config.textConfig
...
/// CPU acceptance loop.
package struct Q35MTPDraftBlock {
    package let tokenIDs: MLXArray
...
package final class Q38MTPModel: Module, Q35MTPDraftModel {
    @ModuleInfo(key: "pre_fc_norm_embedding") var preFCNormEmbedding: Q35RMSNorm
    @ModuleInfo(key: "pre_fc_norm_hidden") var preFCNormHidden: Q35RMSNorm
    ...
    package let diagnosticsID = "qwen4-exp-mtp"
    ...
    @discardableResult
    package func prepareFusedSwitchGLU() -> Bool {
...
package final class Q35MTPDraftSession {
    ...
    package init(promptTokens: [Int] = [], promptHidden: MLXArray? = nil, historyCache: KVCache = KVCacheSimple()) {
    ...
    package var committedHistoryCount: Int {
        historyCache.offset + backlogTokens.count
    }

    package var pendingHistoryCount: Int { backlogTokens.count }

    /// Prefix checkpoints never share a mutable draft cache with a request.
    package func fork() -> Q35MTPDraftSession {
    ...
    package func primeCommittedHistory(mtpModel: any Q35MTPDraftModel, baseModel: Q35Model) {
    ...
    package func recordCommittedTransitions(hiddenStates: MLXArray, nextTokens: [Int]) {
    ...
    package func restoredVerificationState(
