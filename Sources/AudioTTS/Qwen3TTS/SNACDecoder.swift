...
public final class SNACDecoder: Module {
    public enum DecoderError: LocalizedError {
        case noCodesProvided

        public var errorDescription: String? {
            switch self {
            case .noCodesProvided:
                return "SNAC decoder requires at least one code array."
            }
        }
    }
    ...
    public func decode(codes: [[Int]]) throws -> MLXArray {
        ...
        let latent = try quantizer.fromCodes(codes)
    ...
    public func decode(codeArrays: [MLXArray]) throws -> MLXArray {
        let latent = try quantizer.fromCodeArrays(codeArrays)
        return decoder(latent)
    }
}
