...
This guide is for a developer or support operator with an existing ticketing
website open on an Apple Silicon Mac. You can run GLiNER2.5 Decide, Ornith, and
...
2. Select a sample ticket. Record where the window shows its stable ID, status,
   customer message, and context needed for classification.
...
4. Record how the app lists open tickets and how you return to the list. Check
   ...
   navigation step.
5. Choose a read-only way to verify the saved result. Prefer an existing ticket
...
| Stage | What the controller must do |
| --- | --- |
| Discover | List open tickets and identify each ticket by a stable ID. Stop if the selected window or list changes unexpectedly. |
| Observe | Read the selected ticket's message, context, status, and current values. Save the observation before inference. |
| Classify | Pass the observed text and your app's allowed team and priority labels to `mere.run text classify`. Keep the selected labels and scores. |
| Decide | Skip resolved tickets, `No action` results, and requests that need review. Treat GLiNER scores as suggestions rather than calibrated probabilities. |
| Act | Recheck the ticket ID and content. Give the accepted labels and draft-only limits to `mere-computer-use plan`, then run that plan with Ornith. |
| Verify | Read the ticket again. Check team, priority, tags, saved draft, unchanged status, and no sent reply. Stop at the first mismatch. |
| Resume | Before retrying, compare the current ticket with the saved receipt. Skip a completed ticket only after verifying its final state. |
...
The controller uses the website's visible controls for changes. An existing
read-only API can strengthen verification, but the workflow does not require
adding an API or model integration to the ticketing app. The Northline Care
inbox runner is an app-specific, unpublished demo harness; copying its window
labels or ticket IDs would not adapt it to another website.
...
separate app-specific support controller directory in this workspace using
...
For the first run, operate only this fictional or approved sample ticket. Set
team and priority only if its visible content supports the suggestion. Add
...
state independently through an existing read-only API or audit log if
available; otherwise use a fresh window observation and state that limit.
...
After the sample run, review the receipt and the ticket in the website. For a
...
sample ticket has no new sent reply. Check the final state in the app rather
than relying only on the agent's report.
