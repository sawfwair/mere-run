...
final class StudioLiveAcceptanceTests: XCTestCase {
    ...
    /// Per process, so two live passes in different worktrees never read each other's root: a
    /// shared suite name let one pass's `set` redirect the other's destinations mid-test.
    ...
    func test21SeparateWritesStemsAndAManifestFromTheTaskDraft() throws {
        try requireModels(["music-separate-bs-roformer-viperx-1297"])
        let flow = "21-separate"
