...
final class StudioTaskChipsTests: XCTestCase {
    ...
    func testEssentialOptionsBecomeChipsValuedFromTheRecordedCommand() throws {
        ...
        let chips = StudioTaskChips.chips(for: try Self.row(for: draft), titles: .none, source: .contract)
        ...
        XCTAssertEqual(chips.last, StudioModelNaming.displayName(StudioTaskSchema.modelID(for: draft, source: .contract), titles: .none))
    ...
    func testAFoleyRowMirrorsItsComposer() throws {
        ...
        draft.form["--steps"] = .integer(25)
        draft.form["--seed"] = .integer(3)
        XCTAssertEqual(StudioTaskSchema.essentials(for: .soundFoley, draft: draft, source: .contract).map(\.flag), ["--duration", "--steps"])
        let expected = StudioTaskSchema.essentials(for: .soundFoley, draft: draft, source: .contract)
            .filter { $0.overrideID != .variant }
            .map { $0.chipTitle(in: draft) }
            + [StudioModelNaming.displayName(StudioTaskSchema.modelID(for: draft, source: .contract), titles: .none)]
        XCTAssertEqual(StudioTaskChips.chips(for: try Self.row(for: draft), titles: .none, source: .contract), expected)
    ...
    func testAThreeDRowShowsItsEssentialsAndModel() throws {
        ...
        let chips = StudioTaskChips.chips(for: try Self.row(for: draft), titles: .none, source: .contract)
        let essentials = StudioTaskSchema.essentials(for: .threeDFromImage, draft: draft, source: .contract).filter { $0.overrideID != .variant }
        ...
        XCTAssertEqual(StudioTaskSchema.essentials(for: .threeDFromImage, draft: trellis, source: .contract).map(\.flag), ["task-variant", "--seed"])
        XCTAssertTrue(StudioTaskChips.chips(for: try Self.row(for: trellis), titles: .none, source: .contract).contains("Seed 7"))

        let multiview = StudioTaskDraft(templateID: .imageReconstruct3DMultiview)
        XCTAssertEqual(StudioTaskSchema.essentials(for: .threeDFromImage, draft: multiview, source: .contract).map(\.flag), ["task-variant", "--resolution"])
    }
    ...
    func testMusicAnalyzeAndTranscribeShowTheirEssentialSettingsOnCards() throws {
        ...
        XCTAssertEqual(StudioTaskSchema.essentials(for: .musicAnalyze, draft: analyze, source: .contract).map(\.flag), ["--duration"])
        XCTAssertTrue(StudioTaskChips.chips(for: try Self.row(for: analyze), titles: .none, source: .contract).contains("Duration 45"))
        ...
        XCTAssertEqual(StudioTaskSchema.essentials(for: .musicTranscribe, draft: transcribe, source: .contract).map(\.flag), ["--format"])
        let chips = StudioTaskChips.chips(for: try Self.row(for: transcribe), titles: .none, source: .contract)
    ...
    func testARowWithoutARecordedCommandHasNoChips() {
        ...
        XCTAssertEqual(StudioTaskChips.chips(for: row, titles: .none, source: .contract), [])
    }
