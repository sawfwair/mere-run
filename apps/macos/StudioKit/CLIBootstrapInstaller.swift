...
package enum CLIBootstrapInstallError: LocalizedError {
...
package struct CLIManagedInstallation: Equatable, Sendable {
    package let destinationURL: URL
    package let payloadURL: URL
    package let receipt: CLIInstallationReceipt
    package let installedVersion: String
}

package enum CLIBootstrapInstaller {
    package static func installManagedPayload(
        from payloadURL: URL,
    ...
    package static func bundledPayloadURL(
        fileManager fm: FileManager = .default,
        bundle: Bundle = .main
    ) -> URL? {
        let candidates = [
    ...
    package static func preferredAutomaticInstallURL(fileManager fm: FileManager = .default) -> URL {
    ...
    package static func preferredAutomaticInstallURL(
        fileManager fm: FileManager = .default,
    ...
    package static func existingInstallationCandidate(
        paths: CLIInstallationPaths,
        fileManager fm: FileManager = .default
    ) -> URL? {
    ...
    package static func itemExistsIncludingSymbolicLink(_ url: URL, fileManager fm: FileManager = .default) -> Bool {
        fm.fileExists(atPath: url.path) || (try? fm.destinationOfSymbolicLink(atPath: url.path)) != nil
    }

    package static func resolvedSymbolicLink(at url: URL, fileManager fm: FileManager = .default) -> URL? {
    ...
    package static func validatedVersion(executableURL: URL) throws -> String {
    ...
    package static func payloadMatches(
    ...
    package static func destinationIsWritable(_ destinationURL: URL, fileManager fm: FileManager = .default) -> Bool {
