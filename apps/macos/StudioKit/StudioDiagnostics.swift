...
package enum StudioDiagnostics {
    package struct LogSummary: Equatable {
        package var systemCount = 0
        package var stdoutCount = 0
        package var stderrCount = 0

        package var totalCount: Int { systemCount + stdoutCount + stderrCount }
    }

    /// How many recent runs to include. Enough to show a failure in context without
    /// turning the report into a log dump.
    package static let recentRunLimit = 20
    ...
    package static func suggestedFilename(generatedAt: Date = Date()) -> String {
