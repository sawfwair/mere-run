...
package enum StudioMode: String, CaseIterable, Codable, Identifiable {
    case createImage
    case chat
    case code
    case speak
    ...
    package var isConversational: Bool {
        self == .chat || self == .code
    }
    ...
    package var defaultTemplateID: CommandTemplateID {
        switch self {
    ...
    package var acceptedTypes: [UTType] {
        switch self {
    ...
    package var requiresAttachment: Bool {
        switch self {
    ...
    package var promptPlaceholder: String {
        switch self {
    ...
    package var emptyMessage: String {
        switch self {
...
package enum StudioReadImageAction: String, CaseIterable, Codable, Identifiable, Sendable {
    case inspect
    case ocr
    case caption
...
package struct StudioDraft: Codable, Equatable, Sendable {
    /// Every stored property carries a default; this stands in for the synthesized
    ...
    package var preflightJSON = false
    package var cfgScale = 1.0
    package var strength = 0.75
    package var sigmaShift = 0.0
    package var imageMaskPath = ""
    package var imageOutpaintTop = 0
    package var imageOutpaintRight = 0
    package var imageOutpaintBottom = 0
    package var imageOutpaintLeft = 0
    package var imageMaskFeather = 8
    ...
    package var imageMaxSequenceLength = 512
    ...
    package var audioMaxDuration = 0.0
    ...
    package mutating func reset(for mode: StudioMode) {
...
package struct StudioRunRequest: Identifiable, Equatable {
    package let id: UUID
    ...
    package var expectedOutputURL: URL? {
...
package enum StudioCapabilityRequirement: Equatable {
...
package struct StudioMessage: Codable, Identifiable, Equatable {
    package let id: UUID
    package var role: StudioMessageRole
...
package struct StudioLibraryItem: Codable, Identifiable, Equatable {
    package let id: UUID
    package var mode: StudioMode
    package var prompt: String
    package var inputURL: URL?
    package var outputURL: URL?
    package var createdAt: Date
    package var updatedAt: Date
    package var status: StudioLibraryStatus
    ...
    package var commandArguments: [String]? = nil
    ...
    package var artifactURLs: [URL]? = nil
    ...
    package var artifactRoles: [String: String]? = nil
    ...
    package var messages: [StudioMessage]? = nil
    package var systemPrompt: String? = nil
    package var model: String? = nil
    /// Origin for entries imported by an external local launcher. nil means a Studio-owned run.
    package var source: StudioLibrarySource? = nil
    ...
    package var displaySystemImage: String {
        guard let templateID, let template = CommandCatalog.template(id: templateID) else {
    ...
    /// reported none.
    package func artifactRoleLabel(for url: URL) -> String? {
...
package struct StudioModelCapabilityReport: Equatable {
    package let capabilitiesByID: [String: StudioModelCapability]
    package let recommendedChatModelID: String?
    package let recommendedCodeModelID: String?
}

package enum ModelCapabilitiesParser {
    package static func report(from output: String) -> StudioModelCapabilityReport {
    ...
    package static func capabilities(from output: String) -> [String: StudioModelCapability] {
    ...
    private static func jsonReport(from output: String) -> StudioModelCapabilityReport? {
        ...
        struct Payload: Decodable {
            package struct ChatBand: Decodable {
                package let modelID: String
            }
            package struct RecommendedModel: Decodable {
                package let id: String?
                package let modelID: String?
            }
            package struct Model: Decodable {
                package let id: String
            ...
            package let recommendedChatModel: ChatBand?
            package let recommendedCodeModel: RecommendedModel?
            package let models: [Model]
        }
...
package struct StudioRunProgress: Equatable {
...
package enum StudioProgressParser {
    ...
    package static func parse(_ rawLine: String) -> StudioRunProgress? {
...
package enum StudioResultParser {
    /// Output-artifact paths parsed from `stdout`, most-recently-emitted first.
    package static func outputPaths(fromStdout stdout: String) -> [String] {
        var paths: [String] = []
        let lines = stdout
    ...
    package static func isPathLike(_ candidate: String) -> Bool {
...
/// after it.
package struct StudioRunReceipt: Decodable, Equatable {
    ...
    package static let eventName = "result"
    /// How many trailing stdout lines are searched for the receipt.
    package static let trailingLineLimit = 40
    ...
    package static func parse(stdout: String) -> StudioRunReceipt? {
    ...
    package static func strippingReceiptLines(from stdout: String) -> String {
    ...
    package static func isReceiptLine(_ line: String) -> Bool {
        let trimmed = line.trimmingCharacters(in: .whitespacesAndNewlines)
...
package enum StudioArtifactRole {
    package static let structuredPrompt = "structured-prompt"
    package static let timings = "timings"
    package static let detections = "detections"
    package static let tracking = "tracking"
    package static let masks = "masks"
    package static let candidate = "candidate"
    package static let stem = "stem"
    package static let lyrics = "lyrics"
    package static let recipe = "recipe"
    package static let composition = "composition"
    ...
    package static func label(for role: String?) -> String? {
    ...
    package static func inferred(for url: URL, templateID: CommandTemplateID, draft: CommandDraft) -> String? {
...
package enum StudioArtifactDiscovery {
...
package struct StudioServerStatus: Equatable {
    ...
    package var loadedModelSummary: String? {
        loadedModels.first
    }

    package static func parse(jsonStdout: String) -> StudioServerStatus? {
        guard let data = jsonObjectData(in: jsonStdout) else { return nil }
        struct Snapshot: Decodable {
            package struct Server: Decodable {
                package let health: String?
                package let loadedModels: [String]?
            }
...
package struct StudioGuideTopic: Identifiable, Equatable {
    package let id: String
    ...
    package static func parseContent(payloadJSON: String) -> String? {
...
package struct StudioVoiceProfile: Identifiable, Equatable {
    package let id: String
    package let name: String
    package let updatedAt: String

    package static func parse(listOutput: String) -> [StudioVoiceProfile] {
