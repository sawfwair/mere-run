...
package enum MereRunLaunch: Equatable {
    ...
    package func processArguments(for cliArguments: [String]) -> [String] {
        switch self {
        case .executable:
            return cliArguments
        case .swiftRun(let packagePath):
    ...
    package func displayCommand(for cliArguments: [String]) -> String {
        switch self {
        case .executable(let url):
...
package enum CLIResolver {
    package static func resolve(customPath: String) -> MereRunLaunch {
    ...
    package static func existingInstalledCLI(fileManager fm: FileManager = .default) -> URL? {
    ...
    package static func isBundledExecutable(_ url: URL) -> Bool {
        let roots = [
    ...
    package static func installedCandidates(fileManager fm: FileManager) -> [URL] {
        [
    ...
    package static func nearestPackageRoot(from anchor: URL, fileManager fm: FileManager) -> URL? {
...
package struct MereRunUtilityCommandResult: Equatable {
    package let commandPreview: String
    package let exitCode: Int32
    package let stdout: String
    package let stderr: String

    package init(commandPreview: String, exitCode: Int32, stdout: String, stderr: String) {
    ...
    package init(_ result: JobResult) {
        self.init(
...
package final class MereRunController: ObservableObject {
    ...
    @Published package var consoleSeedArguments: [String]?
    @Published package var logs: [LogLine] = []
    @Published package var isRunning = false
    @Published package var status = "Idle"
    @Published package var lastExitCode: Int32?
    @Published package var resolvedCLI = ""
    @Published package var lastOutputURL: URL?
    @Published package var lastRunResult: MereRunRunResult?
    ...
    package let runCompletions = PassthroughSubject<MereRunRunResult, Never>()
    @Published package private(set) var activeRunRequestID: UUID?
    ...
    @Published package private(set) var recommendedCodeModelID: String? = nil {
        didSet {
    ...
    @Published package var runtimeHost: String {
    ...
    @Published package private(set) var progressByRequestID: [UUID: StudioRunProgress] = [:]
    @Published package private(set) var cliVersion: String?
    @Published package private(set) var cliInstallationStatus = CLIInstallationStatus.checking
    ...
    package static let maxConcurrentRuns = JobLane.inference.capacity
    ...
    package static let serverStatusProbeKey = "server-status"
    ...
    package var commandArguments: [String] {
    ...
    package func defaultDraft(for template: CommandTemplate) -> CommandDraft {
    ...
    package func recommendedChatModelForStudioDefault() -> String {
        recommendedChatModelID ?? StudioChatDefaults.fallbackModelID
    }

    package func recommendedCodeModelForStudioDefault() -> String {
        recommendedCodeModelID ?? StudioCodeDefaults.fallbackModelID
    }

    /// Resolves the model a fresh draft starts with: the CLI's recommendation for Chat and Code
    /// over the template default, and over both the model the user made this mode's default on
    /// the Models page.
    ...
    package func syncAdvanced(to mode: StudioMode, from studioDraft: StudioDraft) {
    ...
    package func runtimeURL(path: String) -> URL {
    ...
    package func synchronizeCLIInstallationAfterLaunch() async {
    ...
    package func installCodexSkills() {
    ...
    @discardableResult
    package func saveHuggingFaceToken(_ token: String) async -> Bool {
    ...
    package func loadHuggingFaceEndpoint() async -> String {
    ...
    package func saveHuggingFaceEndpoint(_ endpoint: String) async -> Bool {
    ...
    package func diagnosticsReport(libraryItems: [StudioLibraryItem]) -> String {
    ...
    package func loadConfigurationSummary() async -> String {
    ...
    package func loadConfigurationPath() async -> String {
    ...
    package func loadGuideTopics() async -> [StudioGuideTopic] {
    ...
    package func commandPreview(template: CommandTemplate, draft: CommandDraft, masksSecrets: Bool) -> String {
    ...
    package func commandPreview(arguments: [String], masksSecrets: Bool) -> String {
        let launch = cliResolve(cliPath)
    ...
    @discardableResult
    package func cancelUtilityCommand(_ commandID: UUID) -> Bool {
    ...
    @discardableResult
    package func interruptUtilityCommand(_ commandID: UUID) -> Bool {
    ...
    private func finishCapabilitiesProbe(for mode: StudioMode, result: JobResult) {
        ...
        if result.exitCode != 0, report.capabilitiesByID.isEmpty {
            readinessProbes[mode] = nil
            readinessByMode[mode] = .unknown(Self.capabilitiesUnavailableMessage)
            return
        }
        probeModelList(for: mode)
    }
    ...
    private func probeModelList(for mode: StudioMode) {
        ...
        submitReadinessProbe(for: mode, args: args) { [weak self] result in
            ...
            guard result.exitCode == 0 else {
                readinessByMode[mode] = .unknown(Self.modelListUnavailableMessage)
                return
            }
    ...
    /// What the readiness card says when a probe fails. The CLI's own stderr stays in the probe's
    /// job log; the card tells the user what to do next in plain words.
    nonisolated package static let capabilitiesUnavailableMessage =
        "Couldn't check which models this Mac can run. Check again, or open Models."
    ...
    @discardableResult
    package func runConsole(template: CommandTemplate, draft: CommandDraft, arguments: [String], requestID: UUID?) -> Bool {
        submitInferenceJob(
            template: template,
            draft: draft,
            requestID: requestID,
    ...
    @discardableResult
    package func cancel(requestID: UUID) -> Bool {
        guard let job = jobs.job(requestID: requestID), job.state.isRunning else { return false }
        return jobs.cancel(job.id)
    }

    @discardableResult
    package func submitRealtimeMusicCommand(_ line: String, requestID: UUID) -> Bool {
    ...
    @discardableResult
    package func submitVideoSessionRequest() -> Bool {
    ...
    package func terminateAllProcesses() {
        jobs.terminateAll()
    }
    ...
    package func refreshCLIVersion() {
        Task { @MainActor in
    ...
    package func detectOutputURL(expected: URL?, stdout: String) -> URL? {
