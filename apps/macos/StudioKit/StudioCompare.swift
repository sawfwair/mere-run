...
package enum StudioCompare {
    ...
    /// The first compared file no longer on disk, which Compare names rather than showing an
    /// empty pane; nil when every pane's file is there.
    package static func missingFile(in items: [StudioLibraryItem], fileManager: FileManager = .default) -> URL? {
        items.lazy.compactMap { item in media(of: item).flatMap { url(of: item, as: $0) } }
            .first { !fileManager.fileExists(atPath: $0.path) }
    }

    /// The page Compare opens on for rows starting with `item`: the task that made it when that
    /// page has a canvas to replace (a Generate or Analyze page), else its mode's prompt page —
    /// never a Train, Live, or Voices page, which draw their own surfaces.
    package static func hostTask(for item: StudioLibraryItem) -> StudioTask {
        let task = StudioVariations.submittingTask(for: item)
        return task.showsPromptChrome ? task : item.mode.task
    }
