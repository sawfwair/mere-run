...
package enum StudioAnalyzeDocument: Equatable {
    ...
    /// `subjects`, `metadata`, `stems`), so the shape identifies itself; an RTTM timeline is
    /// read into the diarization document; a payload that is none of those is read as a
    /// transcript.
    ...
    package func detections(imageSize: CGSize, frame: Int = 0) -> [StudioAnalyzeDetection] {
        ...
        case .diarization, .transcript, .pose, .flow, .musicAnalysis, .midi, .clap, .tensor, .separation:
            return []
        }
    }
    ...
    package var speechSegments: [StudioAnalyzeSpeechSegment] {
        ...
        case .diarization(let document):
            return document.segments.enumerated().map { index, segment in
                // A diarized turn has no words, so its row says how long it lasted instead; the
                // row's own left column already says when it started.
                StudioAnalyzeSpeechSegment(
                    id: index,
