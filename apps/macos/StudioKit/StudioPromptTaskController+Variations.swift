...
extension StudioPromptTaskController {
    ...
    @discardableResult
    package func runPromptVariations(seeds: [String]) throws -> StudioVariationSubmission {
        guard let mode = activatedMode, !mode.isConversational else {
            return StudioVariationSubmission(requests: [], outputFallbackReason: nil)
        }
