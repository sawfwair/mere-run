...
/// What one "Run variations" submission started, and why its runs had to move to App Outputs
/// when their folder could not be created.
package struct StudioVariationSubmission {
...
package enum StudioVariations {
    ...
    /// command takes a seed. Threads never do, and neither do the Train pages' runs: eight
    /// copies of a training job are not variations anyone asks for.
    package static func applies(to item: StudioLibraryItem, source: StudioScopeSource) -> Bool {
        guard !item.isConversation, let templateID = item.templateID, templateID.studioTask.archetype != .project,
