...
extension StudioTask {
    ...
    /// renders the archetype's canvas over the task's `StudioTaskDraft` instead. A task not
    /// listed here looks and behaves exactly as before.
    package static let migratedTasks: Set<StudioTask> = [.imageTrain, .chatTrain, .musicTrain]
