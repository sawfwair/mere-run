...
@MainActor
final class StudioJobMonitor: ObservableObject {
    ...
    func isRunning(anyOf requestIDs: [UUID]) -> Bool {
    ...
    var hasInferenceCapacity: Bool {
        store?.hasCapacity(in: .inference) ?? true
    }

    @discardableResult
    func cancel(_ job: Job) -> Bool {
