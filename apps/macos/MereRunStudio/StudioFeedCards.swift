...
enum StudioFeedCardKind: Equatable {
...
struct StudioFeedCard: Identifiable, Equatable {
    let item: StudioLibraryItem
    let kind: StudioFeedCardKind
    /// The row's job while the store still retains it; nil for rows from earlier sessions.
    let job: Job?

    var id: UUID { item.id }

    static func == (lhs: StudioFeedCard, rhs: StudioFeedCard) -> Bool {
...
enum StudioFeedCardBuilder {
    ...
    static func kind(for item: StudioLibraryItem, job: Job?) -> StudioFeedCardKind {
        if let job {
            switch job.state {
    ...
    static func queuePosition(of card: StudioFeedCard, in cards: [StudioFeedCard]) -> Int? {
...
enum StudioFailureSummary {
    ...
    static func summary(outputText: String?, logLines: [String] = [], exitCode: Int32?) -> String {
    ...
    static func isMeaningful(_ rawLine: String) -> Bool {
...
enum StudioRunningStatus {
    static func text(progress: StudioRunProgress?, fallback: String) -> String {
