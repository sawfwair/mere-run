...
@MainActor
final class StudioCrashReporter: NSObject, ObservableObject {
    /// Mirrors the Settings toggle so the app can start and stop capture without a relaunch.
    static let optInDefaultsKey = "studio.diagnostics.metricKitOptIn"

    @Published private(set) var isCapturing = false
    @Published private(set) var storedPayloadCount = 0
    ...
    func refreshStoredPayloadCount() {
