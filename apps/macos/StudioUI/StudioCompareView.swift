...
struct StudioCompareView: View {
    ...
    private var panes: [StudioComparePane] { StudioCompare.panes(for: items, source: scopeSource) }
    ...
    /// pauses. The comparison takes keyboard focus when it opens so both work at once.
...
final class StudioCompareVideoDeck: ObservableObject {
    ...
    func play() {
        if transport.position >= transport.span { seek(to: 0) }
    ...
    func refresh() {
        ...
        if ended { players.forEach { $0.pause() } }
    }
