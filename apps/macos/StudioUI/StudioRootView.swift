...
private struct StudioWorkspaceView: View {
    ...
    private func runComposerVariations(_ count: StudioVariationCount) {
        do {
            let requests = try prompt.runPromptVariations(seeds: StudioVariations.seeds(count: count.rawValue))
            studioError = nil
            navigation.selectedLibraryID = requests.last?.id
        } catch {
            studioError = error.localizedDescription
    ...
    private func openComparison(_ items: [StudioLibraryItem]) {
        ...
        let missing = items.flatMap { item in
            StudioCompare.media(of: item).flatMap { StudioCompare.url(of: item, as: $0) }.map { [$0] } ?? []
        }.filter { !FileManager.default.fileExists(atPath: $0.path) }
        guard missing.isEmpty else {
            studioError = "\(missing[0].lastPathComponent) is no longer on disk."
            return
        }
        let task = StudioVariations.submittingTask(for: first)
        studioError = nil
        libraryOverlay = false
