...
enum CommandFlagsGenerator {
    static let url = URL(fileURLWithPath: #filePath)
        ...
        .deletingLastPathComponent()
        .appendingPathComponent("MereRunStudio/Catalog/CommandFlags.swift")
    ...
    static func render(_ document: MereRunCapabilityDocument) -> String {
        ...
        for capability in document.commands {
            ...
            lines.append("    enum \(namespaceName(for: capability.id)): CommandFlagNamespace {")
            let command = capability.command.map { "\"\($0)\"" }.joined(separator: ", ")
            lines.append("        static let command = [\(command)]")

            let defaults = capability.options.compactMap { option in
                option.defaultValue.map { (option.flag, $0) }
            }
            if !defaults.isEmpty {
                lines.append("        static let defaultValues = [")
            ...
            for option in capability.options {
                lines.append("        static let \(constantName(for: option.flag)) = \"\(option.flag)\"")
            }
            lines.append("    }")
            lines.append("}")
            lines.append("")
