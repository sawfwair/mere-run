...
final class MereRunModelSourceConfigurationTests: XCTestCase {
    ...
    func testArchiveSHA256ParsesConfiguredChecksumMap() {
        let environment = [
            MereRunModelSourceConfiguration.archiveSHA256sEnvironmentKey: """
            models/one.tar.gz=aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa,
            /models/two.tar.gz=bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb
            """
        ]

        XCTAssertEqual(
            MereRunModelSourceConfiguration.archiveSHA256(for: "models/one.tar.gz", environment: environment),
            String(repeating: "a", count: 64)
        )
        XCTAssertEqual(
            MereRunModelSourceConfiguration.archiveSHA256(for: "models/two.tar.gz", environment: environment),
            String(repeating: "b", count: 64)
        )
        XCTAssertNil(
            MereRunModelSourceConfiguration.archiveSHA256(for: "models/missing.tar.gz", environment: environment)
        )
    }
}
