...
final class LagunaComputationTests: MLXTestCase {
    ...
    func testDFlashCancellationPreservesPreparedCachesAndEmitsNoTokens() throws {
        MLXRandom.seed(94)
        let target = LagunaCausalLM(config: try makeConfig())
        let draft = LagunaDFlashModel(config: try makeDFlashConfig())
        let targetCaches = target.makeCache()
        let prefill = target.forward(
            MLXArray([1, 2, 3]).reshaped(1, 3),
            cache: targetCaches,
            captureLayerIndices: [0, 1],
            lastPositionOnly: true
        )
        let draftCaches = draft.makeCache()
        draft.appendTargetContext(
            draft.combineTargetHiddenStates(prefill.capturedHiddenStates),
            cache: draftCaches
        )
        let targetOffsets = targetCaches.map(\.offset)
        let draftOffsets = draftCaches.map(\.offset)
        var emitted: [Int] = []
        XCTAssertThrowsError(try LagunaDFlashDecoder.decode(
            initialLogits: prefill.logits,
            target: target,
            targetCache: targetCaches,
            dflash: draft,
            draftCache: draftCaches,
            generationConfig: GenerationConfig(temperature: 0),
            eosTokens: [],
            tokenBudget: 8,
            historySeedTokens: [1, 2, 3],
            speculativeTokens: 3,
        ...
        XCTAssertEqual(targetCaches.map(\.offset), targetOffsets)
        XCTAssertEqual(draftCaches.map(\.offset), draftOffsets)
    }
}
